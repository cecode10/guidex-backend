...
const FUNCTION_NAME = "updateProfileContent";

const sendError = (res, error) => {
    const statusCode = error?.statusCode || error?.status || 500;
    console.error("%s error: %s", FUNCTION_NAME, error?.message || error);
    let errorCode = error?.message || "update-profile-content failed";
...
export const updateProfileContent = onRequest(
    {
        cors: true,
        region: "europe-west3",
    ...
    async (req, res) => {
        ...
        try {
            ...
            try {
                ...
                console.error(
                    "%s display name lookup failed uid=%s error=%s",
                    FUNCTION_NAME,
                    decoded.uid,
            ...
            const result = await publishProfileContent({
                store,
